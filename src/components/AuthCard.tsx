...
export default function AuthCard({ onLoginSuccess, toast }: any) {
    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            ...
            setStep(2);
            setStatus('Account verified. Enter your Master Password to unlock your vault.');
        } catch (err: any) {
    ...
    const handleEmailAuth = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (authMode === 'email-login') {
                cred = await firebaseEmailLogin(email, emailPass);
            } else {
                cred = await firebaseEmailSignup(email, emailPass);
    ...
    const handleUnlock = async (e: React.FormEvent) => {
        ...
        try {
            ...
            try {
                firebaseData = await loadVault(userId);
                decryptedVault = await performLoginCryptoFlow(password, firebaseData);
            } catch (err: any) {
                if (err.code === 'vault/not-found') {
                    throw new Error('No vault exists for this account yet. Use "First time? Create Vault" to create one.');
                } else if (err.code === 'vault/wrong-master-password') {
                    throw err;
                } else {
                    throw new Error('Could not unlock this vault. Please try again.');
                }
            }
    ...
    const handleCreateVault = async () => {
        ...
        try {
            ...
            try {
                ...
                throw new Error('A vault already exists for this account. Use "Unlock Vault" instead.');
            } catch (err: any) {
            ...
            setStatus('Enter your master password to create or unlock.');
            ...
            <div className="w-full bg-[#161b22]/70 backdrop-blur-xl border border-white/10 rounded-2xl p-8 sm:p-10 shadow-2xl">
                ...
                <div className="flex justify-center mb-6">
                    <div className="w-16 h-16 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center text-primary shadow-[0_0_20px_rgba(88,166,255,0.2)]">
                        {step === 1 ? <ShieldCheck size={32} /> : <Lock size={32} />}
                    </div>
                </div>

                <h2 className="text-2xl font-bold text-center text-white mb-2 tracking-tight">
                    {step === 1 ? 'Verify Account' : 'Unlock Vault'}
                </h2>
                
                <p className="text-sm text-center text-[#8b949e] mb-8 leading-relaxed">
                    {status}
                </p>
                ...
                {step === 1 && (
                    <div className="space-y-6">
                        ...
                        {authMode === 'google' ? (
                            <button 
                                type="button" 
                                className="w-full flex items-center justify-center gap-3 py-3.5 px-4 bg-primary text-white font-medium rounded-xl hover:bg-[#3182ce] active:scale-[0.98] transition-all disabled:opacity-60 shadow-md hover:shadow-[0_0_15px_rgba(88,166,255,0.4)] border border-primary/50" 
                ...
                {step === 2 && (
                    <form onSubmit={handleUnlock} autoComplete="off" className="space-y-5">
                        <div className="relative group">
                            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-[#8b949e] group-focus-within:text-primary transition-colors">
                                <KeyRound size={18} />
                            </div>
                            <input
                                type="password"
                                placeholder="Master Password"
                                className="w-full pl-11 pr-4 py-3.5 bg-[#0d1117]/80 border border-white/10 rounded-xl text-white placeholder-[#8b949e] focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all text-base"
                                value={password} onChange={(e) => setPassword(e.target.value)} disabled={loading} autoFocus
                            />
                        </div>
                        
                        <div className="space-y-3 pt-2">
                            <button type="submit" className="w-full flex items-center justify-center gap-2 py-3.5 px-4 bg-primary text-white font-medium rounded-xl hover:bg-[#3182ce] active:scale-[0.98] transition-all disabled:opacity-60 shadow-md hover:shadow-[0_0_15px_rgba(88,166,255,0.4)] border border-primary/50" disabled={loading}>
                                <ArrowRight size={18} />
                                {loading ? 'Working...' : 'Unlock Vault'}
                            </button>
                            
                            <div className="relative flex items-center py-2">
                                <div className="flex-grow border-t border-white/10"></div>
                                <span className="flex-shrink-0 mx-4 text-[#8b949e] text-xs font-medium uppercase tracking-wider">or</span>
                                <div className="flex-grow border-t border-white/10"></div>
                            </div>

                            <button type="button" className="w-full flex items-center justify-center gap-2 py-3.5 px-4 bg-white/5 text-[#c9d1d9] border border-white/10 font-medium rounded-xl hover:bg-white/10 hover:border-white/20 active:scale-[0.98] transition-all disabled:opacity-60" disabled={loading} onClick={handleCreateVault}>
                                <UserPlus size={18} className="text-[#8b949e]" />
